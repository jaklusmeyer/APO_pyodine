...
def mean(inputData, Cut=3.0):
    """Robust estimator of the mean of a data set
    ...
    if sigmaCut <= 4.5:
        dataSigma = dataSigma / (
            -0.15405 + 0.90723 * sigmaCut - 0.23584 * sigmaCut ** 2.0 + 0.020142
            * sigmaCut ** 3.0)
    ...
    if len(good) > 3:
        dataSigma = math.sqrt(
            ((data[good] - dataMean) ** 2.0).sum() / len(good))
...
def linefit(inputX, inputY, iterMax=25, Bisector=False, BisquareLimit=6.0,
            CloseFactor=0.03):
    """

...
def create_template(utilities, Pars, ostar_files, temp_files, temp_outname, 
    ...
    print('Velocity guess: {0:.3f} km/s'.format(temp_velocity*1e-3))
    print('Barycentric velocity: {0:.3f} km/s'.format(bary_v*1e-3))
    ...
    print('\nTotal number of chunks: ', nr_chunks_total)
    print('Chunks per order: ', nr_chunks_order)
    print('First and last covered pixel of chunks: {}, {}'.format(
            ostar_chunks[0].abspix[0], ostar_chunks[nr_chunks_order-1].abspix[-1]))
    print('Orders: ', ostar_chunks[0].order, ' - ', ostar_chunks[-1].order)
    print()
    ...
    print('\n----------------------')
    print('All runs done, continuing to template deconvolution...\n')
    ...
    if Pars.jansson_lsf_smoothing['do_smoothing'] is True:
        ...
        print('LSFs with nans: ', len(np.unique(np.argwhere(np.isnan(lsf_smoothed_deconv))[:,0])))
    ...
    print('\nDone, full working time: ', modelling_time)
